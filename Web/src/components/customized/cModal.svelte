<!-- Override for Flowbite-Sevlte Modal
 Changes: right-align buttons
-->
<script>
    import classNames from 'classnames';
    import Frame from 'flowbite-svelte/utils/Frame.svelte';
    import {createEventDispatcher} from 'svelte';
    import CloseButton from 'flowbite-svelte/utils/CloseButton.svelte';
    import focusTrap from 'flowbite-svelte/utils/focusTrap';
...
{#if open}
	...
	<div
		...
		class={classNames(
			'fixed top-0 left-0 right-0 z-50 flex h-modal w-full p-4 md:inset-0 md:h-full',
			...getPlacementClasses()
		)}
		tabindex="-1"
		aria-modal="true"
		role="dialog">
		<div class="relative flex {sizes[size]} max-h-full w-full">
			<!-- Modal content -->
			<Frame rounded shadow {...$$restProps} class={frameClass}>
				...
				{#if $$slots.footer}
					<Frame
						color={$$restProps.color}
						class="flex justify-end space-x-2 rounded-b border-t p-6">
						<slot name="footer" />
					</Frame>
				{/if}
			</Frame>
