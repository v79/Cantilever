...
class GeneratorController(val sourceBucket: String) : KoinComponent, APIController {
    ...
    fun generateTemplate(request: Request<Unit>): ResponseEntity<APIResult<String>> {
        ...
        try {
            ...
            info("Checking the ${pageTree.container.count} pages for a template match to $templateKey")
            pageTree.container.children?.filterIsInstance<PageTreeNode.PageMeta>()?.filter { it.templateKey == templateKey }
                ?.forEach {
